...
int main(int argc, char const *argv[]) {
    std::map<int, int> map_with_hint;
    auto it = map_with_hint.begin();
    auto it2 = map_with_hint.begin();

    int mode = std::stoi(argv[1]);
    switch(mode) {
    case 1:  
        for (int i = 0; i < 1000000; ++i) {
            it = map_with_hint.insert(it, std::make_pair(i, i));
        }
        break;

    case 2:  
        for (int i = 0; i < 1000000; ++i) {
            map_with_hint.insert(it, std::make_pair(i, i));
        }
        break;

    case 3:
        it2 = map_with_hint.insert(it, std::make_pair(500000, 500000));
        for (int i = 1000000; i >= 0; --i) {
            if (i != 500000) {
                map_with_hint.insert(it2, std::make_pair(i, i));
            }
        }
        break;
